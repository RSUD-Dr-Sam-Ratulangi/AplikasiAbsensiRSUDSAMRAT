...
const AttendanceConfirmation = ({imageData, navigation, attdType}: any) => {
      ...
      const getScheduledId = async (attendanceDate, employeeId) => {
        await axios
          ...
          .then(response => {
            ...
            const filteredScheduleId = response.data
              .filter(
                schedule =>
    ...
    await convertFileToBase64(imageData)
      .then(({newPath}) => {
        ...
        try {
          ...
          Alert.alert(
            'Error',
            'Pastikan koneksi internet bagus. Jika masih terdapat kendala, mohon hubungi admin.',
  ...
  const checkOut = async attendanceId => {
    await convertFileToBase64(imageData)
      .then(({newPath}) => {
        let url =
          'http://rsudsamrat.site:9999/api/v1/dev/attendances/updatePulang';
        let data = new FormData();
  ...
  const setCheckInOrOut = async (attendanceDate, id) => {
    ...
    try {
      const response = await axios.get(
        `http://rsudsamrat.site:9999/api/v1/dev/attendances/byDateAndEmployee?attendanceDate=${attendanceDate}&employeeId=${id}`,
      );
      ...
      if (
        ...
        setAttendanceInOrOut('Click to Check-In');
      } else {
        const attendanceId = response.data[0].attendanceId;
        setAttendanceId(attendanceId);
        setAttendanceInOrOut('Click to Check-Out');
      }
    } catch (error) {
      console.log(error);
    } finally {
  ...
  const convertFileToBase64 = async filePath => {
    try {
      ...
      return {
        newPath: resize.uri,
        oldFileSize: oldSizeKB,
        newFileSize: newSizeKB,
      };
    } catch (error) {
      throw error;
    }
  ...
  const afterSwipe = async () => {
    try {
      ...
      console.log('Before checkIn or checkOut:', new Date());

      if (attendanceInOrOut === 'Click to Check-In') {
        console.log('Calling checkIn:', new Date());
        await checkIn();
        console.log('checkIn completed:', new Date());
      } else if (attendanceInOrOut === 'Click to Check-Out') {
        console.log('Calling checkOut:', new Date());
        await checkOut(attendanceId);
        console.log('checkOut completed:', new Date());
      }

      console.log('After checkIn or checkOut:', new Date());
      // Any additional logic or state updates after successful check-in or check-out can be placed here.
    } catch (error) {
      // Handle errors appropriately (e.g., display an error message)
      console.error('Error during check-in or check-out:', error);
    } finally {
      ...
      {isLoading ? (
        ...
        <>
          ...
          <TouchableOpacity
            ...
            <Text style={{fontSize: 23, color: '#262D33', fontWeight: '600'}}>
              Take Again
            </Text>
          </TouchableOpacity>
          <View style={{width: '75%', marginTop: 20}}>
            <TouchableOpacity
              ...
              <Text style={{fontSize: 23, color: '#fff', fontWeight: '600'}}>
                {attendanceInOrOut}
              </Text>
            </TouchableOpacity>
          </View>
